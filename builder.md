...
Результат запроса будет возвращен в виде массива. Вы можете получить значение каждого столбца::
...
Метод `order()` позволяет вам отсортировать результат запроса по заданному столбцу. Первый аргумент метода `order()` — столбец для сортировки по нему, а второй — задаёт направление сортировки и может быть либо asc, либо desc:
...
> Если указать только один параметр, то вы установите просто лимит на вывод количества записей.
...
Разумеется, кроме вставки записей в БД конструктор запросов может и изменять существующие строки с помощью метода `update()`. Для использования данного метода необходимо перед этим указать поля для изменения и их новые значения при помощи метода `set()`. Вы можете ограничить запрос update() условием where():
...
Если вы хотите изменить значение **increment** в вашей таблице, то просто используйте метод `increment()` параметр которого принимает новое значение **increment**:

```php 
Builder::table('users')->increment(5);
```
