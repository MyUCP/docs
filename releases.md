...
Так же, начиная с версии <strong>5.5</strong> вы можете указать `callback` функцию которая будет вызвана при переходе на адрес правила. `Callback` функция может быть указан вторым или третьим параметром, но в случае если она указана, то вызов метода контроллера уже не производится.
```php
Router::get("/", function() {
	return view("welcome");
});
...
Вы можете разделить запросы которые будут приходить на ваш сайт на типы <strong>GET</strong> и <strong>POST</strong> тем самым это упростит работу например при использовании ajax, вам не нужно будет проводить лишние проверки на тип запроса.
